...
trait CateCurd
{
    ...
    public function index(CateIndexRequest $request)
    {
        ...
        if (!$admin->can('backend.cate.all')) {
            ...
            $rps = array_get($resourcePermissionConfig, 'resourcePermission');
        ...
        $field = array_column(array_get($indexConfig, 'list'), 'name');
    ...
    {
        $list = array_get($indexConfig, 'list', []);
        ...
        $columns = array_get($columnsConfig, 'column');

        $searchList = array_filter($list, function ($item) {
            return array_get($item, 'search', false);
        });
    ...
    public function order(Builder $query, Request $request, $indexConfig)
    {
        ...
        //读取配置文件
        $defaultOrderBy = array_get($indexConfig, 'orderBy');
        ...
        if ($requestSort === false && array_get($indexConfig, 'orderBy')) {
    ...
    public function store(CateStoreRequest $request)
    {
        ...
        $fields = array_get($createConfig, 'fields');
        $columns = array_get($columnConfig, 'column');
        ...
        try {
            foreach ($fieldsWithColumns as $field) {
                ...
                if (!array_get($field, 'refType')) {
                    //不是关联数据
                    $newModel->$name = $request->input($name);
                } else {
                    switch (array_get($field, 'refType')) {
