...
use App\Http\Controllers\Backend\Users\Request\UsersCreateRequest;
use App\Http\Controllers\Backend\Users\Request\UsersDestroyRequest;
use App\Http\Controllers\Backend\Users\Request\UsersIndexRequest;
use App\Http\Controllers\Backend\Users\Request\UsersShowRequest;
use App\Http\Controllers\Backend\Users\Request\UsersStoreRequest;
use App\Http\Controllers\Backend\Users\Request\UsersUpdateRequest;
...
trait UsersCurd
{
    ...
    private function checkResourcePermission($admin, UsersModel $UsersModel)
    {
        return true;
    }
    ...
    */
    ...
    {
        return $this->success($this->getUsersList($request));
    }
    ...
    */
    public function getUsersList(Request $request){
        $indexConfig = $this->getConfig('Users.PageConfig.Index');
    ...
    public function withRefData(Builder $query, Request $request, $indexConfig)
    {
        ...
        $columnsConfig = $this->getConfig('Users.Column');
    ...
    {
        //批量赋值
        $createConfig = $this->getConfig('Users.PageConfig.Create');
        $columnConfig = $this->getConfig("Users.Column");
    ...
    /**
     * 显示页面
     * @param  UsersShowRequest $request
     * @param  $id
     * @return  \App\Http\Resources\ErrorResource|\App\Http\Resources\SuccessResource
     * @throws  BackendException
    ...
    public function getUsers($id, Request $request){
        $query = UsersModel::query();
        $editConfig = $this->getConfig('Users.PageConfig.Edit');
        $columnConfig = $this->getConfig('Users.Column');
    ...
    /**
     * @param  $id
     * @param  UsersDestroyRequest $request
     * @return  \App\Http\Resources\ErrorResource|\App\Http\Resources\SuccessResource
     */
    public function destroy($id, UsersDestroyRequest $request)
    {
        $model = UsersModel::find($id);
        if (!$model) {
            return $this->failed("模型未找到, id: " . $id);
     ...
     * @throws  \Exception
     */
    public function fillData($fieldsWithColumns, UsersModel $model, Request $request)
    {
        foreach ($fieldsWithColumns as $name => $field) {
            if (!$name) {
                throw new BackendException("Users.PageConfig.Create 配置错误，含有空字段名: " . json_encode($fieldsWithColumns, 256));
            }
