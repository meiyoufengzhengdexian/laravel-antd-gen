...
use App\Http\Controllers\Backend\Product\Request\ProductCreateRequest;
use App\Http\Controllers\Backend\Product\Request\ProductDestroyRequest;
use App\Http\Controllers\Backend\Product\Request\ProductIndexRequest;
use App\Http\Controllers\Backend\Product\Request\ProductShowRequest;
use App\Http\Controllers\Backend\Product\Request\ProductStoreRequest;
use App\Http\Controllers\Backend\Product\Request\ProductUpdateRequest;
...
trait ProductCurd
{
    ...
    private function checkResourcePermission($admin, ProductModel $ProductModel)
    {
        return true;
    }
    ...
    */
    public function index(ProductIndexRequest $request)
    {
        ...
        return $this->success($this->getProductList($request));
    }
    ...
    */
    public function getProductList(Request $request){
        $indexConfig = $this->getConfig('Product.PageConfig.Index');
    ...
    public function withRefData(Builder $query, Request $request, $indexConfig)
    {
        ...
        $columnsConfig = $this->getConfig('Product.Column');
    ...
    {
        //批量赋值
        $createConfig = $this->getConfig('Product.PageConfig.Create');
        $columnConfig = $this->getConfig("Product.Column");
    ...
    {
        $data = $this->getProduct($id, $request);
    ...
    public function getProduct($id, Request $request){
        $query = ProductModel::query();
        $editConfig = $this->getConfig('Product.PageConfig.Edit');
        $columnConfig = $this->getConfig('Product.Column');
     ...
     * @return  \App\Http\Resources\ErrorResource|\App\Http\Resources\SuccessResource
     */
    public function destroy($id, ProductDestroyRequest $request)
    {
        $model = ProductModel::find($id);
        if (!$model) {
            return $this->failed("模型未找到, id: " . $id);
     ...
     * @throws  \Exception
     */
    public function fillData($fieldsWithColumns, ProductModel $model, Request $request)
    {
        foreach ($fieldsWithColumns as $name => $field) {
            if (!$name) {
                throw new BackendException("Product.PageConfig.Create 配置错误，含有空字段名: " . json_encode($fieldsWithColumns, 256));
            }
